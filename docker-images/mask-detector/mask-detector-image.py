...
print("#yolo_load: " + str(datetime.now().strftime("%d-%m-%Y %H:%M:%S.%f")))
net = cv2.dnn.readNetFromDarknet(configPath, weightsPath)

# load our input image and get it height and width
print("#image_read: " + str(datetime.now().strftime("%d-%m-%Y %H:%M:%S.%f")))
image = cv2.imread(args["image"])
(H, W) = image.shape[:2]


# determine only the *output* layer names that we need from YOLO
print("#yolo_compute: " + str(datetime.now().strftime("%d-%m-%Y %H:%M:%S.%f")))
...
print("#process_results: " + str(datetime.now().strftime("%d-%m-%Y %H:%M:%S.%f")))
